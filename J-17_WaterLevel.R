...
end <- as.Date(a$DailyHighDate[1],format='%Y-%m-%d')
bgn <- as.Date(a$DailyHighDate[length(a$DailyHighDate)],format='%Y-%m-%d')
datex <- seq.Date(bgn,end,'day')
pdatex <- as.Date(a$DailyHighDate,format='%Y-%m-%d')

#Check to see if there are missing values
theo <- length(datex)
actu <- length(a$DailyHighDate)
if(theo>actu)print("missing value")

#create a zoo object
WLzoo <- zoo(a$WaterLevelElevation,pdatex)
dum.zoo <-zoo(,datex) #Dummy dataset with time alone
WLzoom <- merge(dum.zoo,WLzoo)
plot(WLzoom,xlab='Year',ylab='WaterLevel(ft)') #see if there are any visible periods of missing records
...
acf(WL.zoof,lag.max=NULL,main='ACF for J-17', type = c('correlation'))
pacf(WL.zoof,main='P.Auto.Cor.Fun J-17',type="o")
